// Step 5: The complete workflow — validate, fetch rate, convert, execute, confirm
import { Effect } from "effect"
import type {
  ConfirmationFailedError, ConvertedAmount, Currency, ExchangeRate,
  InsufficientFundsError, ProviderUnavailableError, RateUnavailableError,
  TransferInput, TransferRejectedError, ValidatedTransfer, ValidationError,
} from "../types"
...
export type SendMoneyDeps = {
  ...
  executeTransfer: (args: {
    recipientIban: string; amount: number; currency: Currency
  }) => Effect.Effect<{ transferId: string }, TransferRejectedError | ProviderUnavailableError>
  sendConfirmation: (args: {
    transferId: string; amount: number; currency: Currency
...
export const createSendMoneyWorkflow = (deps: SendMoneyDeps) => (input: TransferInput) =>
  Effect.gen(function*() {
    ...
    yield* deps.sendConfirmation({
      transferId: transfer.transferId,
      amount: converted.convertedAmount, currency: validated.toCurrency,
    }).pipe(Effect.withSpan("confirm"))
    return { transferId: transfer.transferId, convertedAmount: converted.convertedAmount, rate: rate.rate }
