...
```ts
import { analyze, validateStrict, formatDiagnostics } from "effect-analyzer/rules"
...
| `effect-without-handler` | warning | An effect produces errors but no handler (`catch`, `catchTag`) exists |
...
```ts
import { formatDiagnostics } from "effect-analyzer/rules"
...
ERROR [missing-error-type] Step "getBalance" can fail but has no declared error type
  ...
  Fix: Add an error type annotation or use Effect.catch
...
```ts
import { formatDiagnosticsJSON } from "effect-analyzer/rules"
...
```ts
import { getSummary } from "effect-analyzer/analysis"
