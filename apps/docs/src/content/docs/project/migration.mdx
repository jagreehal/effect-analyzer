...
| `setImmediate` | `Effect.sync` + `queueMicrotask` or `Effect.callback` |
| `fetch()` | `effect/unstable/http` HttpClient |
| `http.request` / `https.request` | `effect/unstable/http` HttpClient |
| `new XMLHttpRequest()` | `effect/unstable/http` HttpClient |
...
| `new Worker()` | `effect/unstable/workers` Worker |
| Class-based DI (`*Service`, `*Repository`, `*Client`) | `Context.Service<Self, Shape>()('Key')` + `Layer.effect` or `Layer.succeed` |
...
| 45 | fetch() | effect/unstable/http HttpClient |
...
```ts
import { findMigrationOpportunities } from "effect-analyzer/migration"
...
```ts
import { findMigrationOpportunitiesInProject, formatMigrationReport } from "effect-analyzer/migration"
