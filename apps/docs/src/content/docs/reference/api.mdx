...
Run a coverage audit across a directory. Returns a `CoverageAuditResult` with
...
Discover and analyze a project once. The returned Project corpus records every
file and its Effect IR programs, zero-program result, analysis failure, and
optional timing. Project-wide views use this shared evidence model.
...
Evaluate native CI expectations against an Audit assessment. Returns a pass or
fail decision with typed violations and does not mutate the measurements.
...
Read [Diagram Fidelity](/effect-analyzer/reference/diagram-fidelity/) for span naming rules and CI usage.
