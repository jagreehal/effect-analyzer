...
| Argument | Behavior |
|---|---|
| `src/transfer.ts` | Analyzes that file, prints the diagram, and writes `src/transfer.effect-analysis.md` |
...
The result goes to stdout; progress, counts and warnings go to stderr. Redirecting produces a file that parses, with the status still visible in the terminal. A single file also writes `*.effect-analysis.md` next to the source unless you pass `--no-colocate`:

```bash
effect-analyze ./src/transfer.ts --format mermaid --no-colocate > transfer.mmd
```
...
| `--include-trivial` | Include trivial programs (schemas, thin wrappers, `Effect.runPromise` entrypoints) that are excluded by default | off |
...
| Command | Description |
|---|---|
| `review` | Review the Effect programs a change touched: structural diff per program, regressions, new lint findings, railway diagrams and a merge-risk verdict, as PR-comment markdown or JSON. What the [GitHub Action](/effect-analyzer/project/github-action/) runs |
...
```bash
effect-analyze review [PATHSPEC...] [--base <ref>] [--head <ref>] [--format markdown|json] [-o <file>] [--fail-on-regression] [--include-tests] [--diagrams open|collapsed]
```

Reads the change from git — `--base` (default `HEAD`) to `--head` (default: the
working tree, untracked files included) — restricted to the given pathspecs. Every
changed `.ts`/`.tsx` file is analyzed at both refs from source; tests, `.d.ts` and
`node_modules` are skipped.

| Flag | Description |
|---|---|
| `--base <ref>` | Ref the change is compared against (default: `HEAD`) |
| `--head <ref>` | Ref holding the change (default: working tree) |
| `--format <fmt>` | `markdown` (default) or `json`. JSON carries `files`, `regressions`, `newFindings`, `risk` and the rendered `markdown` |
| `-o, --output <file>` | Write the report to a file instead of stdout |
| `--fail-on-regression` | Exit 1 when merge risk is `high`: a structural regression or a new lint error |
| `--include-tests` | Also review `*.test.ts` / `*.spec.ts` files (skipped by default) |
| `--diagrams <mode>` | Railway diagrams `open` (default) or `collapsed` in the markdown |

```bash
effect-analyze review --base origin/main src/
effect-analyze review --base main --head HEAD --format json --fail-on-regression
```

Programs are paired by name and order of appearance, diffed with `--diff`'s engine in
regression mode, and both sides are linted so only findings the change introduced are
reported (`info` severity omitted). See [GitHub Action](/effect-analyzer/project/github-action/).
...
A file or a directory writes `.effect-analysis.md` next to each source that contains an Effect program. Linear programs get a railway diagram in that file. `--no-colocate` prints only.

| Flag | Description | Default |
|---|---|---|
| `--colocate` | Write `.effect-analysis.md` next to source (same as the default) | on |
| `--no-colocate` | Print only; do not write adjacent files | — |
| `--colocate-suffix <suffix>` | Custom suffix for colocated files | `effect-analysis` |
| `--no-colocate-enhanced` | Use standard Mermaid instead of railway/enhanced in colocated files | — |
...
These flags are used with the statechart formats (`mermaid-statechart`,
`svg-statechart`, `statechart-html`, `xstate-config`, `statechart-coverage`),
which read [state machines](/effect-analyzer/reference/state-machines/) instead
of the Effect IR:
...
Writes `src/transfer.effect-analysis.md` and prints the diagram. Pass `--no-colocate` to print only.
...
Same as a single file: adjacent `.effect-analysis.md` is the default. Pass `--no-colocate` for a summary only.
