...
<Aside type="note">
The exact numbers were captured during analyzer development before tsconfig path-alias resolution and transitive coupling were available — see the [foldkit case study](/effect-analyzer/case-studies/foldkit/) for examples with both features wired in. The categories of finding and the workflow shown here are unchanged; re-running today produces a fuller picture because `@/...` aliases now resolve to their targets.
</Aside>
