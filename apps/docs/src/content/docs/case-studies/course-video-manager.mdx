...
Suspicious zeros: 0
Failed:     0
Effect adoption:       36.4%
Analysis success:      100.0%
IR source resolution:  95.08%
```
...
Suspicious zeros: 0
Failed:     0
Effect adoption:       73.5%
Analysis success:      100.0%
IR source resolution:  97.74%
```
...
<Aside type="note">
The exact numbers were captured during analyzer development before tsconfig
path-alias resolution, transitive coupling, and the `@effect/tsgo` bridge were
available. The two upstream rule rows below show the current equivalents of the
older raw-side-effect category; re-run with `--tsgo=./tsconfig.json` for current
diagnostics. See the [foldkit case study](/effect-analyzer/case-studies/foldkit/)
for examples with both coupling features wired in.
</Aside>

| Rule | Count | Example |
|---|---|---|
| `nondeterministic-test-api` | 14 | `new Date()` inside test code — flakes waiting to happen |
| `globalDateInEffect` (`@effect/tsgo`) | 6 | Bare `Date.now()` in Effect code — bypasses the `Clock` service |
| `processEnvInEffect` (`@effect/tsgo`) | 5 | `process.env.X` in Effect code — should use `Config.string(...)` |
...
For pnpm/yarn workspace monorepos where sibling packages import each other by name (e.g. `@org/foo`), the API accepts an optional `workspacePackages` map. See the [coupling analyzer reference](/effect-analyzer/reference/coupling/) for details.
