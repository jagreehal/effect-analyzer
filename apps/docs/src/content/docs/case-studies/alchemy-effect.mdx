...
```typescript
import { analyzeCoupling } from 'effect-analyzer/analysis';
...
The lint totals below are a historical analyzer snapshot. For a current run,
add `--tsgo=./packages/alchemy/tsconfig.json`; the former analyzer-only lint
categories are now reported by the official `@effect/tsgo` rules named below.

| Metric | Value |
|---|---|
| Files analyzed | 510 |
| Programs found | 1,534 |
| IR source resolution | 100.0% |
| Lint errors | 0 |
...
- `globalErrorInEffectFailure` (official `@effect/tsgo`) — `Effect.fail(new Error(...))` instead of tagged errors.
- `processEnvInEffect` / `globalDateInEffect` (official `@effect/tsgo`) — `process.env` or `Date.now()` access inside Effect code; these should go through `Config` or `Clock`.
...
- **Real issues worth fixing** — the single `config-secret-without-redacted` finding in `AWS/Environment.ts:22` is unambiguous. Same for `untagged-throw` and the official `globalErrorInEffectFailure` diagnostic.
...
| `--tsgo` | Adds the official Effect language-service diagnostics using the project's TypeScript 7 compiler |
