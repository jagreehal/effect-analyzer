...
Directory mode writes `.effect-analysis.md` next to each source file by default. Each file includes the service dependencies for that file's programs:

```bash
npx effect-analyze ./src
```

Pass `--no-colocate` for a summary only.
