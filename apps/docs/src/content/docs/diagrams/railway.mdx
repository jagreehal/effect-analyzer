...
<DiagramSource
  ...
  A[accounts #lt;- AccountService] -->|ok| B[audit #lt;- AuditLog]
  B -->|ok| C[balance #lt;- accounts.getBalance]
  C -->|ok| D[decision]
  D -->|ok| Done((Success))
  C -->|err| CE[AccountNotFoundError]
...
For the docs transfer fixture, this produces the diagram above. Solid `|ok|` arrows trace the happy path; solid `|err|` arrows show error branches.
