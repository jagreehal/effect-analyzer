...
| Error flow | `--format mermaid-errors` | Producers, what each handler does (caught / mapped / defect / swallowed), what reaches the caller | Reviewing error handling |
...
These read `@typeonce/effect-machine` machines (`Machine.make({...}).handle({...})`)
rather than the Effect IR, so they work on machine-only files that contain no
Effect program. See [State Machines](/effect-analyzer/reference/state-machines/).
...
| XState config | `--format xstate-config` | Paste-ready `createMachine({...})` | [stately.ai/viz](https://stately.ai/viz) import |
...
```bash
# Save to a custom path (skip the adjacent .effect-analysis.md)
npx effect-analyze ./src/program.ts --format mermaid-railway -o diagram.md --no-colocate

# Compact JSON (no indentation)
npx effect-analyze ./src/program.ts --format json --compact --no-colocate

# Control flow direction
npx effect-analyze ./src/program.ts --format mermaid --direction LR --no-colocate

# Include trivial programs (schemas, thin wrappers, Effect.runPromise)
...
- [State Machines](/effect-analyzer/reference/state-machines/) - what the statechart formats read
