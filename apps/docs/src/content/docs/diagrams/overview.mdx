...
That prints the chosen diagrams and writes `program.effect-analysis.md` next to the source. Pass `--no-colocate` to print only.
...
- **Error handling** - a handler touching the error channel triggers the error flow view; when every error simply reaches the caller, the railway view already shows it
- **Service count** - programs with several `Context.Service` dependencies get a service map
...
```ts
import { inferBestDiagramType } from "effect-analyzer/diagram"
...
| `mermaid-errors` | What each handler does to each error; what reaches the caller |
